...
def write_to_json(scene, rows, q_sent_id, q_sta_tok, q_end_tok, a_sent_id, a_sta_tok, a_end_tok, query_mention_id, antecedent_mention_id):
  ...
  if fa_subtitles[q_sent_id] != "": # else, exclude the sentence from the alignment task.
    ...
    if q_head_sta >= 0:
      ...
      rows.append([scene['scene_id'], q_sent_id, q_head_sta, q_head_end, a_sent_id, a_head_sta, a_head_end, query_mention_id, antecedent_mention_id])
  #else:
  ...
  for s in scene['annotations']:
    ...
    if antecedents != ['n', 'o', 't', 'M', 'e', 'n', 't', 'i', 'o', 'n']:
      ...
      if antecedents != ["n", "o", "t", "P", "r", "e", "s", "e", "n", "t"]:
        ...
        write_to_json(scene, rows[scene_id], query['sentenceIndex'], query['startToken'], query['endToken'], -1,-1,-1, query['mention_id'], 'no_antecedent_id')
