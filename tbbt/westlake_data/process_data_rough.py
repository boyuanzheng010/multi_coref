...
def get_label_str(label):
    return \
    {'surprise': 'neutral', 'anger': 'negative', 'fear': 'negative', 'disgust': 'negative', 'sadness': 'negative','distraction': 'negative', 'annoyance':'negative',
     'neutral': 'neutral',
     'trust': 'positive', 'anticipation': 'positive', 'joy': 'positive', None: None}[label]

for field in ['neutral', 'negative', 'positive']:
    temp_data = []
    for x in data:
        if get_label_str(x['target_emotion']) == field:
            temp_data.append(x)
    print(field, len(temp_data))
    # Set path
    file_root = field
